#!/usr/bin/env python3
"""
HIFLD Interactive Search Tool - Jupyter Notebook Version
...
# Connect to ArcGIS - will use existing authentication or prompt
print("\n🔐 Connecting to ArcGIS...")
gis = GIS()  # This will open OAuth browser window if needed
print("✅ Connected!")

if gis.properties.user:
    print(f"👤 Logged in as: {gis.properties.user.username}")
else:
    print("👤 Using anonymous access (limited functionality)")
...
def on_save_click(b):
    ...
    with save_area:
        ...
        suggested_summary = f"Map showing {len(added_layers)} HIFLD infrastructure layers"
        suggested_tags = ['HIFLD', 'infrastructure'] + layer_keywords
        
        print("💾 Save Map Configuration")
        print("=" * 50)
        print(f"\nTitle: {suggested_title}")
        print(f"Summary: {suggested_summary}")
        print(f"Tags: {', '.join(suggested_tags)}")
...
print("\n📝 Try searching for: fire, hospital, school, power")